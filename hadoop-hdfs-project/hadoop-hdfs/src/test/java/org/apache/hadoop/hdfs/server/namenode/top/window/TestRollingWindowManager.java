...
public class TestRollingWindowManager {
  ...
  public void testTops() throws Exception {
    ...
    for (Op op : tops.getOps()) {
      ...
      if (op.getOpType().equals("open")) {
        for (int i = 0; i < topUsers.size(); i++) {
          ...
          assertEquals((users.length-i)*2, user.getCount(), "Unexpected count for user " + user.getUser());
        }
    ...
    for (int i = 0; i < topUsers.size(); i++) {
      User user = topUsers.get(i);
      assertEquals((users.length-i), user.getCount(), "Unexpected count for user " + user.getUser());
    }
    // Closed form of sum(range(1,21))
    assertEquals((1 + users.length) * (users.length / 2), op.getTotalCount(), "Unexpected total count for op");
  }
